...
class Echo():
  ...
  @task
  def long_time_resident_concerns(self) -> Task:
    return Task(
      config=self.tasks_config['long_time_resident_concerns'],
    )
  
  @task
  def long_time_residents_policy_summary(self) -> Task:
    return Task(
      config=self.tasks_config['long_time_residents_policy_summary'],
    )
